...
log_file = '/home/myall1977_2/cryptocurrency/log/btc_alert.log'
...
class get_currency:
	def __init__(self, _api_url):
		...
		self._currency_json = json.loads(_currency_req.content)
	def values(self, _currency_name, _value_name):
		...
		return self._currency_json['data'][_currency_name][_value_name]
	def timestamp(self):
                print self._currency_json            
		return datetime.datetime.fromtimestamp(int(self._currency_json['data']['date'])/1000+32400).strftime('%Y-%m-%d %H:%M:%S')
